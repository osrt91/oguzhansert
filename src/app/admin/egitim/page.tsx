  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-foreground">Eğitim</h1>
        ...
        <div className="rounded-lg border border-border bg-card p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-foreground">
              {editing.id ? "Düzenle" : "Yeni Eğitim"}
            </h2>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              { name: "degree", label: "Derece" },
              { name: "start_date", label: "Başlangıç (YYYY-MM-DD)" },
              { name: "end_date", label: "Bitiş (YYYY-MM-DD)" },
              { name: "period", label: "Dönem" },
              { name: "logo_url", label: "Logo URL" },
              { name: "url", label: "Okul URL" },
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground">Henüz eğitim bilgisi eklenmemiş.</p>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Okul</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Derece</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Dönem</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  <td className="px-4 py-3 text-foreground">{item.school}</td>
