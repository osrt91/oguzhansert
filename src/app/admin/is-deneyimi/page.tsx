...
const EMPTY: Partial<WorkExperience> = {
  company: "",
  title: "",
  start_date: "",
  end_date: "",
...
export default function IsDeneyimiPage() {
  const { items, loading, fetchItems, createItem, updateItem, deleteItem } =
    useAdmin<WorkExperience>("work_experience");
  ...
  const toggleVisibility = async (item: WorkExperience) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-foreground">İş Deneyimi</h1>
        <Button
          onClick={() => {
            setEditing({ ...EMPTY });
            setShowForm(true);
        ...
        <div className="rounded-lg border border-border bg-card p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-foreground">
              {editing.id ? "Düzenle" : "Yeni İş Deneyimi"}
            </h2>
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground">Henüz iş deneyimi eklenmemiş.</p>
      ) : (
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  <td className="px-4 py-3 text-foreground">{item.company}</td>
